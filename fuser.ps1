...
function Test-EdidBlockChecksum{param([byte[]]$b)$ok=$true;for($i=0;$i -lt $b.Length;$i+=128){$s=0;for($j=0;$j -lt [Math]::Min(128,$b.Length-$i);$j++){$s+=$b[$i+$j]}if(($s%256)-ne 0){$ok=$false}}$ok}
...
function Get-EdidInfo{
    ...
    [PSCustomObject]@{BytesLen=$len;ChecksumOk=$chk;SerialASCII=$serialAscii;SerialHEX=$serialHex;SerialNum=$serialNum;Year=$year;Week=$week;Mfg=$mfg;ProductCode=$prod;Model=$name;ZeroSerial=$allZero}
}
...
function Test-IsLaptop{
    $isLap=$false
    try{$enc=Get-CimInstance Win32_SystemEnclosure -ErrorAction SilentlyContinue;if($enc){$ct=@($enc.ChassisTypes);$hot=@(8,9,10,14,30,31);if(@($ct|Where-Object{$hot -contains $_}).Count -gt 0){$isLap=$true}}}catch{}
    try{$cs=Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue;if($cs){if($cs.PCSystemType -eq 2){$isLap=$true}}}catch{}
    try{$bat=Get-CimInstance Win32_Battery -ErrorAction SilentlyContinue;if($bat){$isLap=$true}}catch{}
    return $isLap
}
...
if($fuser){Write-Host "Fuser found" -ForegroundColor Red}else{Write-Host "No Fuser found" -ForegroundColor Green}
